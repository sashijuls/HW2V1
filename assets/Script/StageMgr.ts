...
import { KeyControl } from "../Function/types";
...
export default class StageMgr extends cc.Component {
    ...
    start() {
        // initialize game index (offline mode — always use fresh defaults)
        this.coinNum = 0;
        this.score = 0;
        this.life = 5;
    ...
    // save best score to local storage (offline, no account needed)
    safePoint() {
        const bestScore = Number(cc.sys.localStorage.getItem('MARIO_BEST_SCORE') || '0');
